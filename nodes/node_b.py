"""
Node B - IoT Device dengan Edge Processing dan Offline Mode (IMPROVED VERSION)
Fitur Baru:
- Communication latency tracking
- Throughput monitoring
- Quality of Service (QoS) metrics
- Detailed offline performance analytics
"""

import paho.mqtt.client as mqtt
import json
import time
import random
import threading
from datetime import datetime
from typing import Optional
from collections import deque

class NodeBImproved:
    def __init__(self):
        self.broker = "localhost"
        self.port = 1883
        self.client: Optional[mqtt.Client] = None
        self.is_connected = False
        self.is_online_mode = True
        self.running = True
        self.lock = threading.Lock()
        self.retry_count = 0
        self.max_retries = 5
        self.offline_decisions = 0
        
        # Edge processing - data buffer
        self.sensor_buffer = deque(maxlen=100)
        self.processed_data = {}
        
        # NEW: Communication metrics
        self.messages_sent = 0
        self.messages_failed = 0
        self.message_latencies = deque(maxlen=100)
        self.avg_latency = 0
        self.min_latency = None
        self.max_latency = None
        
        # NEW: Throughput tracking
        self.throughput_window = deque(maxlen=60)  # Last 60 seconds
        self.current_throughput = 0
        self.peak_throughput = 0
        
        # NEW: Quality of Service metrics
        self.qos_metrics = {
            "delivery_success_rate": 100.0,
            "avg_processing_time": 0,
            "data_loss_events": 0
        }
        
        # NEW: Offline mode metrics
        self.offline_mode_count = 0
        self.offline_durations = deque(maxlen=50)
        self.offline_start_time = None
        self.total_offline_time = 0
        
        # Topics
        self.topic_data = "/nodeB/data"
        self.topic_status = "/nodeB/status"
        self.topic_metrics = "/nodeB/metrics"
        
    def on_connect(self, client, userdata, flags, rc):
        """Callback saat koneksi berhasil"""
        if rc == 0:
            print(f"[Node B] ✅ Connected to Broker (rc={rc})")
            with self.lock:
                self.is_connected = True
                self.is_online_mode = True
                self.retry_count = 0
                
                # Track offline duration
                if self.offline_start_time:
                    offline_duration = time.time() - self.offline_start_time
                    self.offline_durations.append(offline_duration)
                    self.total_offline_time += offline_duration
                    print(f"[Node B] 🔄 Back online after {offline_duration:.2f}s offline")
                    self.offline_start_time = None
        else:
            print(f"[Node B] ❌ Connection failed with code {rc}")
            with self.lock:
                self.is_connected = False
    
    def on_disconnect(self, client, userdata, rc):
        """Callback saat disconnect"""
        print(f"[Node B] ⚠️ Disconnected from Broker (rc={rc})")
        with self.lock:
            self.is_connected = False
            self.is_online_mode = False
            self.offline_mode_count += 1
            if not self.offline_start_time:
                self.offline_start_time = time.time()
        
        if rc != 0:
            print(f"[Node B] 🔧 Unexpected disconnect! Entering OFFLINE MODE...")
            self.retry_connection()
    
    def on_publish(self, client, userdata, mid):
        """Callback saat publish berhasil"""
        with self.lock:
            self.messages_sent += 1
            self._update_qos_metrics()
    
    def _update_qos_metrics(self):
        """Update QoS metrics"""
        total = self.messages_sent + self.messages_failed
        if total > 0:
            self.qos_metrics["delivery_success_rate"] = (self.messages_sent / total) * 100
    
    def connect_to_broker(self) -> bool:
        """Koneksi ke broker"""
        try:
            self.client = mqtt.Client(
                client_id=f"node_b_{int(time.time())}",
                callback_api_version=mqtt.CallbackAPIVersion.VERSION1
            )
            self.client.on_connect = self.on_connect
            self.client.on_disconnect = self.on_disconnect
            self.client.on_publish = self.on_publish
            
            print(f"[Node B] 🔌 Connecting to {self.broker}:{self.port}...")
            self.client.connect(self.broker, self.port, keepalive=60)
            self.client.loop_start()
            
            # Wait for connection
            timeout = 5
            start_time = time.time()
            while not self.is_connected and (time.time() - start_time) < timeout:
                time.sleep(0.1)
            
            return self.is_connected
        except Exception as e:
            print(f"[Node B] ❌ Connection error: {e}")
            return False
    
    def retry_connection(self):
        """Retry koneksi dengan fallback"""
        with self.lock:
            self.retry_count += 1
        
        print(f"[Node B] 🔄 Retry connection attempt {self.retry_count}/{self.max_retries}...")
        
        # Exponential backoff
        wait_time = min(2 ** self.retry_count, 30)
        time.sleep(wait_time)
        
        success = self.connect_to_broker()
        
        if success:
            print(f"[Node B] ✅ Reconnection SUCCESS after {self.retry_count} attempts!")
            with self.lock:
                self.is_online_mode = True
                self.retry_count = 0
            self.publish_status("reconnected", f"Back online after {self.retry_count} retries")
            self.publish_metrics()
        else:
            if self.retry_count < self.max_retries:
                print(f"[Node B] ⚠️ Retry failed, attempting again...")
                self.retry_connection()
            else:
                print(f"[Node B] ❌ Max retries reached. Staying in OFFLINE MODE.")
                with self.lock:
                    self.retry_count = 0
                    self.is_online_mode = False
    
    def edge_processing(self, sensor_data: dict) -> dict:
        """Edge Processing dengan performance tracking"""
        process_start = time.time()
        
        self.sensor_buffer.append(sensor_data)
        
        # Hitung statistik dari buffer
        if len(self.sensor_buffer) >= 5:
            temps = [d['temperature'] for d in self.sensor_buffer]
            humids = [d['humidity'] for d in self.sensor_buffer]
            
            avg_temp = sum(temps) / len(temps)
            avg_humid = sum(humids) / len(humids)
            min_temp = min(temps)
            max_temp = max(temps)
            
            # Deteksi anomali
            anomaly = sensor_data['temperature'] > 32.0
            
            # Keputusan lokal
            if anomaly:
                decision = "ALERT: High temperature detected!"
                action = "cooling_on"
                priority = "high"
            elif avg_temp > 28.0:
                decision = "WARNING: Average temperature elevated"
                action = "monitor"
                priority = "medium"
            else:
                decision = "NORMAL: All parameters within range"
                action = "none"
                priority = "low"
            
            processing_time = time.time() - process_start
            
            # Update processing time metrics
            with self.lock:
                if self.qos_metrics["avg_processing_time"] == 0:
                    self.qos_metrics["avg_processing_time"] = processing_time
                else:
                    self.qos_metrics["avg_processing_time"] = \
                        (self.qos_metrics["avg_processing_time"] * 0.9) + (processing_time * 0.1)
            
            processed = {
                "raw_data": sensor_data,
                "statistics": {
                    "avg_temperature": round(avg_temp, 2),
                    "avg_humidity": round(avg_humid, 2),
                    "min_temperature": round(min_temp, 2),
                    "max_temperature": round(max_temp, 2),
                    "samples": len(self.sensor_buffer)
                },
                "anomaly_detected": anomaly,
                "decision": decision,
                "action": action,
                "priority": priority,
                "processing_time": round(processing_time * 1000, 2),  # ms
                "processed_at": datetime.now().isoformat()
            }
            
            self.processed_data = processed
            return processed
        else:
            return {
                "raw_data": sensor_data,
                "message": "Collecting data for edge processing...",
                "samples": len(self.sensor_buffer),
                "processing_time": round((time.time() - process_start) * 1000, 2)
            }
    
    def publish_status(self, event_type: str, message: str):
        """Publish status ke broker"""
        if self.client and self.is_connected:
            payload = {
                "node": "B",
                "event": event_type,
                "message": message,
                "mode": "online" if self.is_online_mode else "offline",
                "retry_count": self.retry_count,
                "offline_decisions": self.offline_decisions,
                "timestamp": datetime.now().isoformat()
            }
            try:
                self.client.publish(self.topic_status, json.dumps(payload), qos=1)
            except Exception as e:
                print(f"[Node B] ❌ Publish status error: {e}")
    
    def publish_metrics(self):
        """Publish detailed metrics"""
        if self.client and self.is_connected:
            avg_offline_duration = sum(self.offline_durations) / len(self.offline_durations) \
                if self.offline_durations else 0
            
            payload = {
                "node": "B",
                "communication_metrics": {
                    "messages_sent": self.messages_sent,
                    "messages_failed": self.messages_failed,
                    "avg_latency": round(self.avg_latency * 1000, 2),  # ms
                    "min_latency": round(self.min_latency * 1000, 2) if self.min_latency else None,
                    "max_latency": round(self.max_latency * 1000, 2) if self.max_latency else None
                },
                "throughput_metrics": {
                    "current_throughput": self.current_throughput,
                    "peak_throughput": self.peak_throughput
                },
                "qos_metrics": self.qos_metrics,
                "offline_metrics": {
                    "offline_mode_count": self.offline_mode_count,
                    "offline_decisions": self.offline_decisions,
                    "total_offline_time": round(self.total_offline_time, 2),
                    "avg_offline_duration": round(avg_offline_duration, 2)
                },
                "timestamp": datetime.now().isoformat()
            }
            try:
                self.client.publish(self.topic_metrics, json.dumps(payload), qos=1)
            except Exception as e:
                print(f"[Node B] ❌ Publish metrics error: {e}")
    
    def publish_data(self, data: dict):
        """Publish data dengan latency tracking"""
        if self.client and self.is_connected:
            send_time = time.time()
            data["send_time"] = send_time
            
            try:
                result = self.client.publish(self.topic_data, json.dumps(data), qos=1)
                if result.rc == mqtt.MQTT_ERR_SUCCESS:
                    # Track latency
                    latency = time.time() - send_time
                    with self.lock:
                        self.message_latencies.append(latency)
                        self.avg_latency = sum(self.message_latencies) / len(self.message_latencies)
                        
                        if self.min_latency is None or latency < self.min_latency:
                            self.min_latency = latency
                        if self.max_latency is None or latency > self.max_latency:
                            self.max_latency = latency
                        
                        # Track throughput
                        self.throughput_window.append(time.time())
                        recent_messages = sum(1 for t in self.throughput_window 
                                            if time.time() - t <= 1.0)
                        self.current_throughput = recent_messages
                        if recent_messages > self.peak_throughput:
                            self.peak_throughput = recent_messages
                    
                    return True
                else:
                    with self.lock:
                        self.messages_failed += 1
                        self._update_qos_metrics()
                    return False
            except Exception as e:
                print(f"[Node B] ❌ Publish data error: {e}")
                with self.lock:
                    self.messages_failed += 1
                    self._update_qos_metrics()
                return False
        return False
    
    def sensor_and_processing_thread(self):
        """Thread untuk sensor + edge processing"""
        print("[Node B] 📡 Sensor and processing thread started")
        
        while self.running:
            # Simulate sensor reading
            sensor_data = {
                "node": "B",
                "temperature": round(random.uniform(20.0, 35.0), 2),
                "humidity": round(random.uniform(40.0, 80.0), 2),
                "light": round(random.uniform(100.0, 1000.0), 2),
                "timestamp": datetime.now().isoformat()
            }
            
            # Edge processing (selalu jalan, online atau offline)
            processed = self.edge_processing(sensor_data)
            
            # Tambahkan info mode
            processed["mode"] = "online" if self.is_online_mode else "offline"
            processed["node"] = "B"
            
            if self.is_online_mode and self.is_connected:
                # Online mode: publish ke broker
                success = self.publish_data(processed)
                if success:
                    print(f"[Node B] [ONLINE] ✅ Data published | "
                          f"Decision: {processed.get('decision', 'N/A')[:30]} | "
                          f"Latency: {self.avg_latency*1000:.2f}ms")
                else:
                    print(f"[Node B] [ONLINE] ❌ Publish failed, switching to offline mode")
                    with self.lock:
                        self.is_online_mode = False
            else:
                # Offline mode: keputusan lokal saja
                with self.lock:
                    self.offline_decisions += 1
                
                print(f"[Node B] [OFFLINE] 🔧 Decision #{self.offline_decisions}: "
                      f"{processed.get('decision', 'N/A')[:40]}")
                
                # Coba reconnect setiap 10 keputusan offline
                if self.offline_decisions % 10 == 0:
                    print(f"[Node B] 🔄 Attempting reconnection after {self.offline_decisions} offline decisions...")
                    threading.Thread(target=self.retry_connection, daemon=True).start()
            
            time.sleep(3)
    
    def status_monitor_thread(self):
        """Thread untuk monitoring status"""
        print("[Node B] 🏥 Status monitor thread started")
        
        while self.running:
            time.sleep(10)
            
            mode = "ONLINE" if self.is_online_mode else "OFFLINE"
            status = "Connected" if self.is_connected else "Disconnected"
            
            print(f"[Node B] 📊 Status: {mode} | {status} | "
                  f"Throughput: {self.current_throughput} msg/s | "
                  f"QoS: {self.qos_metrics['delivery_success_rate']:.1f}%")
            
            if self.is_connected:
                self.publish_status("heartbeat", f"Mode: {mode}, Status: {status}")
    
    def metrics_reporter_thread(self):
        """Thread untuk report metrics"""
        print("[Node B] 📊 Metrics reporter started")
        while self.running:
            time.sleep(15)
            if self.is_connected:
                self.publish_metrics()
    
    def start(self):
        """Start Node B"""
        print("=" * 70)
        print("Node B - IoT Device with Edge Processing & Offline Mode (IMPROVED)")
        print("=" * 70)
        
        # Connect ke broker
        success = self.connect_to_broker()
        
        if not success:
            print("[Node B] ⚠️ Broker unavailable, starting in OFFLINE MODE...")
            with self.lock:
                self.is_online_mode = False
                self.offline_start_time = time.time()
        
        # Start threads
        processing_t = threading.Thread(target=self.sensor_and_processing_thread, daemon=True)
        monitor_t = threading.Thread(target=self.status_monitor_thread, daemon=True)
        metrics_t = threading.Thread(target=self.metrics_reporter_thread, daemon=True)
        
        processing_t.start()
        monitor_t.start()
        metrics_t.start()
        
        print("[Node B] ✅ All threads started. Press Ctrl+C to stop.")
        print("\n📊 Improvements:")
        print("  - Communication latency tracking")
        print("  - Throughput monitoring (msg/s)")
        print("  - Quality of Service metrics")
        print("  - Detailed offline analytics\n")
        
        try:
            while self.running:
                time.sleep(1)
        except KeyboardInterrupt:
            print("\n[Node B] 🛑 Stopping...")
            self.running = False
            if self.client and self.is_connected:
                self.publish_status("shutdown", "Node B shutting down")
                self.publish_metrics()
                self.client.loop_stop()
                self.client.disconnect()
            
            # Print final statistics
            print("\n" + "=" * 70)
            print("FINAL STATISTICS")
            print("=" * 70)
            print(f"Messages Sent: {self.messages_sent}")
            print(f"Messages Failed: {self.messages_failed}")
            print(f"Success Rate: {self.qos_metrics['delivery_success_rate']:.2f}%")
            print(f"Avg Latency: {self.avg_latency*1000:.2f}ms")
            print(f"Peak Throughput: {self.peak_throughput} msg/s")
            print(f"Offline Decisions: {self.offline_decisions}")
            print(f"Total Offline Time: {self.total_offline_time:.2f}s")
            print(f"Offline Mode Count: {self.offline_mode_count}")
            print("=" * 70)
            print("[Node B] ✅ Stopped.")

if __name__ == "__main__":
    node = NodeBImproved()
    node.start()
